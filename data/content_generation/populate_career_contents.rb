  ...
  def save_to_database(results)
    results.each do |code, data|
      ActiveRecord::Base.connection.execute <<~SQL
        ...
        VALUES ('#{code}', '#{data[:summary]}', '#{data[:full]}', '#{data[:video_url]}', NOW(), NOW())
  ...
  def save_images_to_database(images_results)
    images_results.each do |code, data|
      data[:images].each_with_index do |img, idx|
        ActiveRecord::Base.connection.execute <<~SQL
          INSERT INTO career_images (occupation_code, image_url, prompt_used, "order", created_at, updated_at)
          VALUES ('#{code}', '#{img[:url]}', '#{img[:prompt]}', #{idx}, NOW(), NOW())
        SQL
      end
    end
  end
